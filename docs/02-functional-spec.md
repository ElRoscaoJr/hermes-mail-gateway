...
- Outputs: normalized message summaries or requested bounded content, with opaque references, stable message metadata, thread headers, attachment metadata, and provider-independent dates where available. Raw provider response objects are never returned.
