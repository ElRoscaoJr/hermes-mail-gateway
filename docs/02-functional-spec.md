...
- Inputs: `accountId`, operation (`folders`, `list`, `search`, `read`, `thread`, `attachments`, or `verifySent`), optional folder, query string, strict provider-neutral `search` filters, opaque message reference, pagination cursor, bounded limit, and optional zero-based `attachmentIndex` for attachment download.
...
- Outputs: for `folders`, safe folder metadata only (`path`, `name`, `delimiter`, and optional `specialUse`); for other operations, normalized message summaries or requested bounded content, with opaque references, stable message metadata, and provider-independent dates where available. Attachment-only queries return bounded filename/content type/size metadata; with `attachmentIndex`, the selected safe attachment returns filename, content type, size, SHA-256, and base64 content only. Thread queries return bounded normalized summaries matched by server-side Message-ID, In-Reply-To, and References criteria. Raw provider response objects, raw MIME, and arbitrary headers are never returned.
...
- Structured search filters are strict and reject unknown fields. `from`, `to`, `cc`, `subject`, `since`, `before`, `isRead`, `isFlagged`, and `messageId` compile to server-side IMAP criteria. `hasAttachment` is explicitly unsupported when the adapter cannot represent it safely; no unbounded client-side scan is performed. Query strings and filters remain bounded and server-side, with existing UIDVALIDITY cursor semantics.
- Attachment download is bounded by configured `maxReadBytes` and a conservative 5,000,000-byte public maximum. Out-of-range, stale, cross-account, oversized, inline, content-ID, missing/unsafe metadata, and unsupported attachments fail safely.
