...
- Outputs: normalized message summaries or requested bounded content, with opaque references, stable message metadata, and provider-independent dates where available. Bounded `read` includes attachment metadata; `thread` and attachment-only query operations are explicit `UNSUPPORTED_OPERATION` contracts. Raw provider response objects are never returned.
