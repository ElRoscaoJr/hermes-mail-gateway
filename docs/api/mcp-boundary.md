...
It resolves every account explicitly, returns only safe account projections, bounds list/search/thread results to the configured inbox or Sent folder, dispatches only to the adapter registered for that account, and delegates message preparation and non-verification execution to the existing durable domain functions. `verifyOnly` verifies the persisted Message-ID without calling SMTP. `attachments` reads an account-scoped opaque message reference and returns only bounded filename, content type, and size metadata. `thread` reads the anchor, then uses one bounded server-side IMAP header search for its Message-ID, In-Reply-To, and References IDs; results include the anchor, deduplicate UIDs, and are limited before return. Account configuration fields (`credentialRef`, endpoints, attachment roots, and policy), raw MIME, and attachment bytes never appear in its public results.
