...
export const mailPrepareSchema = z.strictObject({ accountId: id, idempotencyKey: z.string().min(8).max(200), fromAddress: z.string().email().optional(), recipients: z.array(z.string().email()).min(1).max(100), subject: z.string().max(998), textBody: z.string().max(1_000_000).optional(), htmlBody: z.string().max(1_000_000).optional(), attachments: z.array(z.strictObject({ path: z.string().min(1).max(4096), sha256: z.string().regex(/^[a-f0-9]{64}$/), contentType: z.string().max(128) })).max(32).default([]) });
