...
export const accountConfigSchema = z.strictObject({ accountId: z.string().regex(/^[a-z0-9][a-z0-9_-]{1,63}$/), displayName: z.string().min(1).max(100), providerKind: z.enum(["gmail", "zoho", "generic_imap_smtp"]), imapEndpoint, smtpEndpoint, credentialRef: z.string().min(1).max(200), smtpCredentialRef: z.string().min(1).max(200).optional(), sentPolicy: z.literal("provider_managed"), enabled: z.boolean(), allowedSender: z.string().email(), allowedAttachmentRoots: z.array(z.string().min(1)).max(32), inboxFolder: z.string().min(1).max(200), sentFolder: z.string().min(1).max(200) });
